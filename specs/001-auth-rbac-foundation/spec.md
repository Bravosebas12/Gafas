...
- **FR-003a**: Toda contraseña DEBE tener entre **8 y 12 caracteres**, ambos inclusive. El rango se valida allí donde la contraseña se establece —creación del primer Administrador, y creación y cambio en las features RF-USR y RF-CFG—. En el ingreso solo se valida el tope superior, para no revelar la política ni romper el mensaje genérico de FR-004: una contraseña fuera del rango se rechaza como credencial incorrecta, no como error de formato.
- **FR-003b**: El sistema DEBE aceptar cualquier carácter dentro de ese rango, incluidos espacios, acentos y caracteres no latinos, y almacenarlos sin corrupción ni truncamiento.
...
- **FR-009**: La credencial de renovación DEBE expirar 8 horas después de la **autenticación** que originó la sesión, de modo que cubra una jornada laboral completa.
- **FR-009a**: El vencimiento de la credencial de renovación DEBE ser **absoluto respecto al inicio de la sesión**: la rotación de FR-011 emite una credencial nueva que **hereda la fecha de vencimiento** de la credencial consumida, sin recalcularla. Cumplidas las 8 horas, el usuario DEBE autenticarse de nuevo con su contraseña.
...
- **FR-029**: Los roles DEBEN asignarse a la **cuenta de usuario**, no al perfil de empleado. Como cada usuario pertenece a exactamente un empleado, los roles quedan atribuibles al empleado a través de su cuenta, sin duplicar la relación. Un empleado sin cuenta de usuario no tiene roles ni acceso.
...
- **FR-038**: El sistema DEBE escribir en la tabla de auditoría **exactamente** estos eventos, y esta lista es exhaustiva para la feature: bloqueo de cuenta (FR-021), cambio de roles (FR-028), **cierre de sesión**, **revocación en cascada de credenciales por reutilización** (FR-013) y **creación del primer Administrador** (FR-035, con usuario responsable nulo, por ser acción de un operador y no de la aplicación). La renovación de sesión DEBE quedar en el registro estructurado de FR-037 y **no** en la tabla de auditoría: ocurre cada 15 minutos por usuario, unas 30 veces por jornada, y no aporta información que la traza de intentos y el log no tengan.
...
- **Decidido 2026-08-20:** el vencimiento de la credencial de renovación es **absoluto** desde la autenticación y la rotación hereda la fecha, sin recalcularla (FR-009, FR-009a). Sin esta regla, una cadena de rotaciones cada 15 minutos mantendría la sesión viva de forma indefinida y el límite de 8 horas no significaría nada. **Consecuencia operativa**: un turno de más de 8 horas exige autenticarse una segunda vez. Si el negocio opera turnos más largos, el valor a subir es el de FR-009, no la regla de herencia.
- **Decidido 2026-08-20:** la longitud de la contraseña queda fijada entre **8 y 12 caracteres** (FR-003a), por decisión del responsable del proyecto. **Desviación consciente de los estándares**: NIST SP 800-63B exige que el verificador admita al menos 64 caracteres y OWASP ASVS recomienda lo mismo, de modo que un tope de 12 impide el uso de frases de contraseña, que son la vía más simple para obtener una credencial fuerte y memorable. El riesgo residual se considera acotado por dos mecanismos ya presentes: el bloqueo tras cinco intentos fallidos (FR-017), que hace impracticable el ataque en línea, y las 600.000 iteraciones de derivación (D-01), que encarecen el ataque fuera de línea si la base se filtrara. **Si el sistema se expone a internet o se audita contra un marco de cumplimiento, este tope DEBE revisarse.** El valor a cambiar es el de FR-003a; nada en el esquema ni en el algoritmo depende de él, porque la derivación es de tamaño fijo.
- **Decidido 2026-08-20:** la limitación de intentos **por origen de la petición** queda **fuera del alcance** de esta feature. El bloqueo de FR-017 protege cuenta por cuenta, de modo que un ataque repartido entre muchas cuentas distintas no dispara ningún bloqueo. Se acepta el riesgo porque el sistema opera en una sola sede, con del orden de diez usuarios internos y sin exposición pública. **Si la aplicación se expone a internet, esta exclusión DEBE revisarse antes del despliegue.**
- **Decidido 2026-08-20:** FR-029 se corrigió para asignar los roles a la cuenta de usuario y no al perfil de empleado. La redacción original contradecía el modelo de datos, que es fuente de verdad según el principio X, y habría obligado a un cambio de esquema sin ninguna necesidad funcional.
