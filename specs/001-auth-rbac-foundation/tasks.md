...
- [x] T015 [P] Crear los tipos base del dominio en `src/1. Core/Optica.Domain/Comun/`: resultado de operación, excepciones de dominio y tipos de valor, todos inmutables (principio V)
- [ ] T016 [P] Declarar las abstracciones de la capa de aplicación en `src/1. Core/Optica.Application/Abstracciones/`: repositorios, `IHasheadorDeContrasena`, `IEmisorDeToken` y unidad de trabajo. **Parcial**: se declararon `IUnidadDeTrabajo`, `IEmisorDeToken` e `IHasheadorDeContrasena`. Los repositorios de entidad NO se pueden declarar aquí: `IRepositorioDeUsuarios` necesita la entidad `Usuario`, que llega en T033. Cada repositorio se declara junto a su entidad, en la tarea que la crea. Es un defecto de orden de tasks.md, no un olvido
- [x] T017 Registrar MediatR y los comportamientos de canal en `src/1. Core/Optica.Application/Comportamientos/`: validación previa al handler, log con identificador de correlación y transacción
- [x] T018 Crear `src/2. Infrastructure/Optica.Infrastructure/Persistencia/OpticaDbContext.cs` **sin migraciones**, con las configuraciones mapeadas a las tablas existentes de `ADMINISTRACION_USUARIOS` y `AUDITORIA` (decisión D-03, principio X)
- [x] T019 Implementar el interceptor de auditoría en `src/2. Infrastructure/Optica.Infrastructure/Persistencia/Interceptores/AuditoriaInterceptor.cs`, que escribe en `AUDITORIA.LogUsuarios` dentro de la misma transacción y **excluye de forma explícita** `PASSWORD_HASH`, `PASSWORD_SALT` y `TOKEN_HASH` (compuerta G7, regla R-A3). Escribe **exactamente los cuatro eventos de FR-038** y ninguno más: bloqueo de cuenta, cambio de roles, cierre de sesión y revocación en cascada por reutilización de credencial. La renovación de sesión NO va a la tabla de auditoría, solo al registro estructurado (decisión D-14)
- [x] T020 Implementar el interceptor que puebla `USUARIO_CREACION`, `FECHA_CREACION`, `USUARIO_ACTUALIZACION` y `FECHA_ACTUALIZACION` desde `TimeProvider` y el usuario de la petición (principio VII)
- [x] T021 [P] Registrar `TimeProvider` en la inyección de dependencias y prohibir `DateTime.Now` y `DateTime.UtcNow` mediante una regla del analizador (decisión D-07)
- [x] T022 Implementar el validador de configuración en `src/2. Infrastructure/Optica.Infrastructure/Configuracion/ValidadorDeProveedoresExternos.cs`, que **falla el arranque** ante cualquier proveedor de identidad externo declarado (FR-002, compuerta G5)
- [x] T023 Validar en el arranque que la clave de firma del token existe y mide al menos 32 bytes, fallando de forma explícita si no (decisión D-08). El algoritmo de firma es **HS256** con clave simétrica única, sin rotación programada en esta feature (decisión D-08a)
